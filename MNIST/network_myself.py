...
def online_train(train_data,train_label,w1,w2,b1,b2,eta):
	for i in range(train_data.shape[0]):
		deta1,delta_w2,deta2,delta_w1=back_propagation(train_data[i,:].reshape((1,-1)),train_label[i,:].reshape((1,-1)),w1,w2,b1,b2,eta)
		w2=w2-delta_w2*eta
		b2=b2-deta1*eta
		w1=w1-delta_w1*eta
		b1=b1-deta2*eta
		...
		if (i+1)%5000==0:
			print("traing times:%d"%(i+1))

	return w1,w2,b1,b2

def batch_trian(train_data,train_label,w1,w2,b1,b2,eta):
...
def mini_batch_train(train_data,train_label,w1,w2,b1,b2,eta,mini_batch_size):
	mini_batch_index=range(train_data.shape[0])
	random.shuffle(mini_batch_index)
	mini_batch_index=np.array(mini_batch_index).reshape((-1,mini_batch_size))

	deta1=np.zeros((mini_batch_size,b2.shape[1]))
	deta2=np.zeros((mini_batch_size,b1.shape[1]))
	delta_w1=np.zeros((w1.shape[0],w1.shape[1],mini_batch_size))
	delta_w2=np.zeros((w2.shape[0],w2.shape[1],mini_batch_size))

	for k in range(mini_batch_index.shape[0]):
		w1,w2,b1,b2=batch_trian(train_data[mini_batch_index[k,:]],train_label[mini_batch_index[k,:]],w1,w2,b1,b2,eta)

		if (k+1)%1000==0:
			print("mini_batch trianing:%d"%(k+1))
	return w1,w2,b1,b2
...
def trian(train_data,train_label,input_dim,h,output_dim,eta,trian_model):
	...
	if trian_model=='online':
		...
		epoch=5
		for i in range(epoch):
			w1,w2,b1,b2=batch_train(train_data,train_label,w1,w2,b1,b2,eta)
			print("the training epoch:%d"%(i+1))
		return w1,w2,b1,b2
	else :
		epoch=5
		mini_batch_size=10
		for i in range(epoch):
			w1,w2,b1,b2=mini_batch_train(train_data,train_label,w1,w2,b1,b2,eta,mini_batch_size)
			print("the training epoch:%d"%(i+1))
		return w1,w2,b1,b2
